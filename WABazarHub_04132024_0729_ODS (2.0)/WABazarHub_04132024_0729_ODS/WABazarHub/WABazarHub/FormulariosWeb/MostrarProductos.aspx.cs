...
namespace WABazarHub.FormulariosWeb
{
    public partial class MostrarProductos : System.Web.UI.Page
    {
        ...
        private CProvedores cProvedores;
        ...
        protected string ObtenerNombreProveedor(object proveedorID)
        {
            int id;
            if (int.TryParse(proveedorID.ToString(), out id))
            {
                CProvedores cProveedores = new CProvedores();
                List<EProveedores> proveedores = cProveedores.ObtenerTodosProveedores();
                EProveedores proveedor = proveedores.FirstOrDefault(p => p.ProveedorID == id);

                // Si se encuentra el proveedor, devolver su nombre; de lo contrario, devolver un mensaje indicando que no se encontró
                return proveedor != null ? proveedor.Contacto : "Proveedor no encontrado";
            }

            return "Proveedor no especificado";
        }
        
        private EProveedores ObtenerProveedorMas(int proveedorID, List<EProveedores> proveedores)
        {
            // Suponiendo que EProveedores tiene una propiedad IDProveedor que coincide con el ID del proveedor asociado al producto
            return proveedores.FirstOrDefault(p => p.ProveedorID == proveedorID);
        }
